...
Normalerweise wird ein Server nach der Instalaltion automatisch gestartet. Wir starten ihn manuell:
...
Dies nennt man relative Adresssierung da wir in das Verzeichnis wechseln in dessen Elternverzeichnis wir uns gerade beinden. Die relative Adressierung stimmt nur an dem Ort an dem wir uns gerade befinden. Die absolute Adressierung fängt immer mit einem "/" an. In unserem Fall würde der Befehl absolut dann heißen:
...
Es kanns ein, dass die Installation des Servers Nginx durch den laufenden Apache2 gestört wurde. in disem Fall startet der Nginx nicht und muss gelöscht und neu installiert werden.
...
Bitte installiere nginx erneut. Achte darauf, dass der Apache2 nicht läuft.
...
Um alle Pakete zu sehen, die aktuell auf dem Computer laufen, wir ebenfalls dpkg verwendet:
